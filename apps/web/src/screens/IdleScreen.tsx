...
export function IdleScreen({ onStart, onOpenGallery }: IdleScreenProps) {
  return (
    <main className="relative min-h-screen overflow-hidden bg-brand-red text-white">
      ...
      <div className="relative z-10 min-h-screen" onClick={onStart}>
        <div className="mx-auto flex min-h-screen max-w-[1920px] flex-col px-8 py-6 lg:px-16 lg:py-8">
          ...
          <section className="grid flex-1 items-center gap-12 py-10 lg:grid-cols-[1.15fr_0.85fr]">
            <div className="max-w-5xl">
              ...
              <button
                className="mt-12 inline-flex min-h-20 items-center rounded-full bg-white px-12 py-5 text-3xl font-bold text-brand-red shadow-2xl lg:text-4xl"
                type="button"
                onClick={(event) => {
            ...
            <div onClick={(event) => event.stopPropagation()}>
              <HopeCarousel onOpenGallery={onOpenGallery} />
            </div>
          </section>
        </div>
      </div>
